...
* `vnext`：一个数组，包含一系列的服务器配置，其中每一个服务器：
  ...
  * `users`：一组服务器认可的用户，其中每一个用户：
    ...
    * `level`: 用户等级
    * `security`：加密方式，客户端将使用配置的加密方式发送数据。服务器端无需配置，自动识别。可选的值有：
      * `"aes-128-cfb"`
...
* `disableInsecureEncryption` (V2Ray 3.11+): 禁止客户端使用不安全的加密方式，默认值为`false`。
  （当服务端此值为`true`时并且客户端指定下列加密方式时，服务器会主动断开连接。）
  * `none`
  * `aes-128-cfb`
